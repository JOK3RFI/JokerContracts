...
contract FRAXStablecoin is ERC20Custom, AccessControl, Owned {
    ...
    address public black_eth_oracle_address;
    ...
    function oracle_price(PriceChoice choice) internal view returns (uint256) {
        ...
        else if (choice == PriceChoice.BLACK) {
            price_vs_eth = uint256(blackEthOracle.consult(weth_address, PRICE_PRECISION)); // How much FXS if you put in PRICE_PRECISION WETH
        }
    ...
    function fxs_price()  public view returns (uint256) {
     ...
     // Returns X BLACK = 1 USD
    function black_price()  public view returns (uint256) {
        return oracle_price(PriceChoice.BLACK);
    }
    ...
    // for testing the collateralratio
    function setglobal_collateral_ratio(uint256 _global_collateral_ratio)  external  {
    ...
    function setBlackEthOracle(address _black_oracle_addr, address _weth_address) public onlyByOwnerGovernanceOrController {
        require((_black_oracle_addr != address(0)) && (_weth_address != address(0)), "Zero address detected");

        black_eth_oracle_address = _black_oracle_addr;
        blackEthOracle = UniswapPairOracle(_black_oracle_addr);
        weth_address = _weth_address;

        emit FXSEthOracleSet(_black_oracle_addr, _weth_address);
    }
