...
contract FraxPoolV3 is Owned {
    ...
    uint256 private chainlink_black_usd_decimals;

    ERC20 public blackAddress;
    ...
    constructor (
        ...
        blackAddress = ERC20(black_address);
        ...
        chainlink_black_usd_decimals = priceFeedBLACKUSD.decimals();
    }
    ...
    function getBLACKPrice() public view returns (uint256) {
        (uint80 roundID, int price, , uint256 updatedAt, uint80 answeredInRound) = priceFeedBLACKUSD.latestRoundData();
        require(price >= 0 && updatedAt!= 0 && answeredInRound >= roundID, "Invalid chainlink price");

        return uint256(price).mul(PRICE_PRECISION).div(10 ** chainlink_black_usd_decimals);
    }
    ...
    /* ========== PUBLIC FUNCTIONS ========== */
     ...
     function mintFrax(
        ...
        uint256 fxs_needed,
        uint256 black_needed
    ) {
        uint256 fraxAmount = frax_amt;
        uint256 colID = col_idx;
        ...
        if (one_to_one_override || global_collateral_ratio >= PRICE_PRECISION) { 
            ...
            collat_needed = getFRAXInCollateral(colID, frax_amt);
            fxs_needed = 0;
        } else if (global_collateral_ratio == 0) { 
            // Algorithmic
            collat_needed = 0;
            uint256 splited_value = (fraxAmount.mul(50)).div(100);
            // fxs_needed = frax_amt.mul(PRICE_PRECISION).div(getFXSPrice());
            fxs_needed = splited_value.mul(PRICE_PRECISION).div(getFXSPrice());
            black_needed = (fraxAmount.sub(splited_value)).mul(PRICE_PRECISION).div(getBLACKPrice());
        } else { 
            // Fractional
            uint256 frax_for_collat = fraxAmount.mul(global_collateral_ratio).div(PRICE_PRECISION);
            uint256 frax_for_fxs = fraxAmount.sub(frax_for_collat);
            collat_needed = getFRAXInCollateral(colID, frax_for_collat);
            uint256 splited_value = (frax_for_fxs.mul(50)).div(100);
            fxs_needed = splited_value.mul(PRICE_PRECISION).div(getFXSPrice());
            black_needed = (frax_for_fxs.sub(splited_value)).mul(PRICE_PRECISION).div(getBLACKPrice());
        }

        // Subtract the minting fee
        total_frax_mint = (fraxAmount.mul(PRICE_PRECISION.sub(minting_fee[colID]))).div(PRICE_PRECISION);
        ...
        require(freeCollatBalance(colID).add(collat_needed) <= pool_ceilings[col_idx], "Pool ceiling");

        // Take the FXS and collateral first
        FXS.pool_burn_from(msg.sender, fxs_needed);
        TransferHelper.safeTransferFrom(collateral_addresses[colID], msg.sender, address(this), collat_needed);
        if(black_needed > 0){
            blackAddress.transferFrom( msg.sender, address(this), (black_needed.div(1e9)));
        }
    ...
    function redeemFrax(
        ...
        if(global_collateral_ratio >= PRICE_PRECISION) { 
            ...
            // Algorithmic
            uint256 splited_value = (frax_after_fee.mul(50)).div(100);
            fxs_out = splited_value.mul(PRICE_PRECISION).div(getFXSPrice());
            black_out = (frax_after_fee.sub(splited_value)).mul(PRICE_PRECISION).div(getBLACKPrice());
            collat_out = 0;
        } else { 
            // Fractional
            collat_out = getFRAXInCollateral(col_idx, frax_after_fee)
                            ...
                            .div(PRICE_PRECISION);
            uint256 splited_value = (frax_after_fee.mul(50)).div(100);
            fxs_out = splited_value
                            .mul(PRICE_PRECISION.sub(global_collateral_ratio))
                            .div(getFXSPrice()); // PRICE_PRECISIONS CANCEL OUT
            black_out = (frax_after_fee.sub(splited_value))
                            .mul(PRICE_PRECISION.sub(global_collateral_ratio))
                            .div(getBLACKPrice()); // PRICE_PRECISIONS CANCEL OUT                
        }
        ...
        require((fxs_out+(black_out.div(1e9))) >= fxs_out_min, "FXS slippage");
        ...
        blackAddress.transfer(msg.sender, (black_out.div(1e9)));
    }
    ...
    function buyBackFxs(uint256 col_idx, uint256 fxs_amount, uint256 col_out_min) external collateralEnabled(col_idx) returns (uint256 col_out) {
        ...
        uint256 splited_value = (fxs_amount.mul(50)).div(100);

        // Make sure not to take more than is available
        uint256 fxs_dollar_value_d18 = splited_value.mul(fxs_price).div(PRICE_PRECISION);
        uint256 black_dollar_value_d18 = (fxs_amount.sub(splited_value)).mul(getBLACKPrice()).div(PRICE_PRECISION);
        ...
        uint256 collateral_equivalent_d18 = (fxs_dollar_value_d18.add(black_dollar_value_d18.div(1e9))).mul(PRICE_PRECISION).div(collateral_prices[col_idx]);
        ...
        FXS.pool_burn_from(msg.sender, fxs_dollar_value_d18);
        blackAddress.transferFrom( msg.sender, address(this), (black_dollar_value_d18.div(1e9)));
    ...
    function recollateralize(uint256 col_idx, uint256 collateral_amount, uint256 fxs_out_min) external collateralEnabled(col_idx) returns (uint256 fxs_out) {
        ...
        fxs_out = collateral_amount_d18.mul(PRICE_PRECISION.add(bonus_rate).sub(recollat_fee[col_idx]));
        uint256 fxs_amount = ((fxs_out.mul(50)).div(100)).div(fxs_price);
        uint256 black_amount = ((fxs_out.mul(50)).div(100)).div(getBLACKPrice());

        // Make sure there is FXS available
        require(fxs_out <= fxs_actually_available, "Insuf FXS Avail For RCT");

        // Check slippage
        require((fxs_amount + (black_amount.div(1e9))) >= fxs_out_min, "FXS slippage");
        ...
        blackAddress.transfer(msg.sender, (black_amount.div(1e9)));
        // Increment the outbound FXS, in E18
        // Used for recollat throttling
        rctHourlyCum[curEpochHr()] += fxs_amount;
    }
    ...
    function setOracles(address _frax_usd_chainlink_addr, address _fxs_usd_chainlink_addr, address _black_usd_chainlink_addr) external onlyByOwnGov {
        // Set the instances
        ...
        priceFeedBLACKUSD = AggregatorV3Interface(_black_usd_chainlink_addr);
        // Set the decimals
        chainlink_frax_usd_decimals = priceFeedFRAXUSD.decimals();
        chainlink_fxs_usd_decimals = priceFeedFXSUSD.decimals();
        chainlink_black_usd_decimals = priceFeedBLACKUSD.decimals();
