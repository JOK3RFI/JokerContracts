...
contract FraxPool is AccessControl, Owned {
    ...
    function mintAlgorithmicFRAX(uint256 fxs_amount_d18, uint256 FRAX_out_min) external notMintPaused {
        ...
        uint256 black_price = FRAX.black_price();
        require(FRAX.global_collateral_ratio() == 0, "Collateral ratio must be 0");
        
        uint256 fxs_value = (fxs_amount_d18 * 70 )/100;
        uint256 black_value = fxs_amount_d18 - fxs_value;
        ...
        (uint256 black_amount) = FraxPoolLibrary.calcMintAlgorithmicFRAX(
            black_price, // X FXS / 1 USD
            // fxs_amount_d18
            black_value
        );

        uint256 mint_fee_fxs = (minting_fee * 30)/100 ;
        uint256 black_with_price = (black_amount.mul(uint(1e6).sub(mint_fee_fxs))).div(1e6);
        uint256 fxs_with_price = (frax_amount_d18.mul(uint(1e6).sub((minting_fee - mint_fee_fxs)))).div(1e6);
        uint256 frax_amount_mint = fxs_with_price + black_with_price;

        // frax_amount_d18 = (frax_amount_d18.mul(uint(1e6).sub(minting_fee))).div(1e6);
        require(FRAX_out_min <= frax_amount_d18, "Slippage limit reached");
        
        FXS.pool_burn_from(msg.sender, fxs_with_price);
        FRAX.pool_mint(msg.sender, frax_amount_mint);
        blackAddress.transferFrom( msg.sender, address(this), (black_with_price.div(1e9)));
    }
    ...
    function mintFractionalFRAX(uint256 collateral_amount, uint256 fxs_amount, uint256 FRAX_out_min) external notMintPaused {
        ...
        uint256 fxs_need_value = (fxs_needed.mul(70)).div(100);
        uint256 balance_fxs = fxs_needed.sub(fxs_need_value);
        uint256 fxs_withoutPrice = (balance_fxs.mul(1e6)).div(fxs_price);

        (uint256 black_amount) = FraxPoolLibrary.calcMintAlgorithmicFRAX(
            FRAX.black_price(), // X FXS / 1 USD
            // fxs_amount_d18
            fxs_withoutPrice
        );

        uint256 col_amount = collateral_amount;
        FXS.pool_burn_from(msg.sender, fxs_need_value);
        TransferHelper.safeTransferFrom(address(collateral_token), msg.sender, address(this), col_amount);
        FRAX.pool_mint(msg.sender, mint_amount);
        blackAddress.transferFrom( msg.sender, address(this), (black_amount.div(1e9)));
    }
    ...
    function redeemFractionalFRAX(uint256 FRAX_amount, uint256 FXS_out_min, uint256 COLLATERAL_out_min) external notRedeemPaused {
        ...
        //splitted fxs into fxs + black
        uint256 blackAmount_needed = (fxs_dollar_value_d18.mul(30)).div(100);
        uint256 fxs_amount = (fxs_dollar_value_d18 - blackAmount_needed).mul(PRICE_PRECISION).div(fxs_price);
        ...
        uint256 black_amount = (blackAmount_needed.mul(PRICE_PRECISION)).div(FRAX.black_price());

        blackAddress.transfer(msg.sender, (black_amount.div(1e9)));
        uint256 fraxAmount = FRAX_amount;
        // Move all external functions to the end
        FRAX.pool_burn_from(msg.sender, fraxAmount);
    ...
    function recollateralizeFRAX(uint256 collateral_amount, uint256 FXS_out_min) external {
        ...
        uint256 fxs_needed = (amount_to_recollat.mul(70)).div(100);
        uint256 fxs_paid_back = (fxs_needed).mul(uint(1e6).add(bonus_rate).sub(recollat_fee)).div(fxs_price);


        //split fxs into DIME+BLACK
        
        
        uint256 black_amount = ((fxs_paid_back.sub(fxs_needed)).mul(PRICE_PRECISION)).div(FRAX.black_price());
        
        require(FXS_out_min <= (fxs_paid_back.add(black_amount)), "Slippage limit reached");
        blackAddress.transfer(msg.sender,(black_amount.div(1e9)));
        

        
        TransferHelper.safeTransferFrom(address(collateral_token), msg.sender, address(this), collateral_units_precision);
        FXS.pool_mint(msg.sender, fxs_needed);
    ...
    function buyBackFXS(uint256 FXS_amount, uint256 COLLATERAL_out_min) external {
        ...
        uint256 fxs_value = (FXS_amount * 70)/100; 
        uint256 black_value = FXS_amount.sub(fxs_value);
        ...
        FraxPoolLibrary.BuybackFXS_Params memory input_params1 = FraxPoolLibrary.BuybackFXS_Params(
            availableExcessCollatDV(),
            black_price,
            getCollateralPrice(),
            black_value
        ...
        (uint256 collateral_equivalent1_d18) = (FraxPoolLibrary.calcBuyBackFXS(input_params)).mul(uint(1e6).sub(buyback_fee)).div(1e6);
        (uint256 collateral_equivalent2_d18) = (FraxPoolLibrary.calcBuyBackFXS(input_params1)).mul(uint(1e6).sub(buyback_fee)).div(1e6);

        // uint256 collateral_precision = collateral_equivalent1_d18.div(10 ** missing_decimals);

        require(COLLATERAL_out_min <= (collateral_equivalent1_d18.add(collateral_equivalent2_d18)), "Slippage limit reached");
        // Give the sender their desired collateral and burn the FXS
        FXS.pool_burn_from(msg.sender, FXS_amount);
        TransferHelper.safeTransfer(address(collateral_token), msg.sender, (collateral_equivalent1_d18.add(collateral_equivalent2_d18)));
    }
    ...
    function setFraxAddress(address _fraxAddress) external onlyByOwnGov {
        frax_contract_address = _fraxAddress;
