...
contract BlackCollateral is Context, IBEP20, Ownable,Initializable {
 ...
 /**
   ...
   function _changeFreezeBlock(address account, uint256 _block) external onlyOwner() {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (overMinTokenBalance && !inSwapAndLiquify && sender != uniswapV3Pair && swapAndLiquifyEnabled) 
        {           
            //add liquidity
			...
			if (_isrewardExcluded[sender] && !_isrewardExcluded[recipient]) {
				    ...
				    if((sender  == uniswapV3Pair)||(stakingAddress[recipient] == true))
				    {
 ...
 function swapAndLiquify(uint256 liquidityBalanceWithContract) private lockTheSwap {
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {
        ...
        // make the swap
        uniswapV3Router.swapExactTokensForETH(
            tokenAmount,
    ...
    */
    ...
    //to receive BNB from uniswapV3Router when swapping
